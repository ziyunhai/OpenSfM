...
import numpy as np
from opensfm import features as features_module
...
def run_dataset(dataset: DataSetBase, rolling_shutter_readout: Optional[float]) -> None:
    ...
    for reconstruction in reconstructions:
        ...
        if len(all_shots) > 0:
            ...
            if compute_speed:
                ...
                for cur_shot in all_shots[1:]:
                    ...
                    if cur_camera_id != prev_camera_id:
                        ...
                        exifs[cur_shot]["speed"] = (
                            cur_shot_origin - prev_shot_origin
                        ) / delta_time
                    else:
                        exifs[cur_shot]["speed"] = np.array([0.0, 0.0, 0.0])
                        logger.warning(
                            "Cannot compute velocity for %s (delta time 0)" % cur_shot
                        )
            ...
            features: Dict[str, Optional[features_module.FeaturesData]] = {}
            feature_ids: Dict[str, Dict[int, bool]] = {}

            for track_id in reconstruction.points:
                # Observations of this track for shots in this reconstruction
                obs = {
                    k: v
                    for k, v in tracks_manager.get_track_observations(track_id).items()
                    if k in reconstruction.shots
                }
                ...
                for shot_id in obs:
                    ...
                    rs_time = (
                        exifs[shot_id].get("rolling_shutter", rolling_shutter_readout)
                        / 1000.0
                    )
                    origin = shot.pose.get_origin()

                    new_origin = origin - exifs[shot_id]["speed"] * rs_time * (
                        pixel_y - shot.camera.height / 2.0
                    ) / shot.camera.height
                    ...
                    feats = features[shot_id]
                    if feats:
                        feats.points[ob.id][:2] = corrected_point
                        feature_ids[shot_id][ob.id] = True

            for shot_id, feats in features.items():
                if feats is None:
                    continue
                ids = np.array(list(feature_ids[shot_id].keys()))
                feats.points = feats.points[ids]
                if feats.descriptors is not None:
                    feats.descriptors = feats.descriptors[ids]
                feats.colors = feats.colors[ids]
                if feats.semantic is not None:
                    feats.semantic = feats.semantic.mask(ids)

                logger.info("Writing corrected and trimmed features for %s" % shot_id)
                dataset.save_features(shot_id, feats)
        else:
