...
intensity ``I``::
    ...
    log I  =  alpha_i  +  sum_k gamma_{i,k} * rho^{2k}   (  ~  ref_t  )
  ...
  ref_t          the track's reference (consensus) log-colour

The ONLY unknowns are the per-image corrections (gain + vignetting) — exactly
what is applied downstream.  The track reference ``ref_t`` is NOT a free
variable: given the current corrections it is just the robust weighted mean of
the track's corrected observations.  So the fit is solved by simple alternating
minimisation, wrapped in IRLS / Huber reweighting (specular highlights, moving
objects and mis-tracks are down-weighted):

  1. recompute each track's reference ``ref_t`` = robust mean of its corrected
     observations;
  2. refit every image's ``(alpha_i, gamma_i)`` to its own observations against
     those references — a tiny independent ``(1 + V)``-unknown least squares per
     image (no cross-image linear system, so it scales linearly with the set);
  3. re-centre the log-gains to ``mean(alpha) = 0`` (the global scale is
     unobservable; this keeps the mean brightness unchanged).
...
def _features(rho2: NDArray, vignette_order: int) -> NDArray:
    """Per-observation design row ``[1, rho^2, rho^4, …, rho^(2V)]`` (Nobs, 1+V).

    Column 0 is the gain (constant) term; columns 1..V are the vignette basis.
    """
    v = int(vignette_order)
    feats = np.ones((len(rho2), 1 + v), dtype=np.float64)
    power = rho2.copy()
    for k in range(v):
        feats[:, 1 + k] = power
        power = power * rho2
...
def _solve_channel(
    ...
    """Alternating IRLS fit of one channel's per-image gain + vignetting.

    Returns ``x`` of shape ``(n_img, 1 + V)``: column 0 is ``alpha_i`` (log-gain),
    columns 1..V the vignette coefficients ``gamma_{i,k}``.  Each iteration
    recomputes the robust per-track reference, refits every image independently
    against it (a small normal-equation solve per image), and re-centres the
    gains; the Huber weights are refreshed from the fit residuals between
    iterations.
    """
    v = int(vignette_order)
    m = 1 + v
    iters = max(1, int(config["equalize_irls_iterations"]))
    ...
    x = np.zeros((n_img, m), dtype=np.float64)
    w = base_w.copy()
    for it in range(iters):
        # Corrected log-colour under the current per-image model, then the
        # robust per-track reference = weighted mean of those corrected values.
        model = np.einsum("om,om->o", feats, x[img])
        corrected = y - model
        wsum = np.bincount(trk, weights=w, minlength=n_trk)
        rsum = np.bincount(trk, weights=w * corrected, minlength=n_trk)
        ref = np.zeros(n_trk, dtype=np.float64)
        nz = wsum > 0.0
        ref[nz] = rsum[nz] / wsum[nz]

        # Refit each image to its observations: feats @ x_i ≈ y - ref_t.  The
        # images are decoupled (the reference is fixed within the iteration), so
        # this is one independent (m×m) normal-equation solve per image, built
        # by scattering the observation outer-products into per-image blocks.
        target = y - ref[trk]
        fw = feats * w[:, None]
        A = np.zeros((n_img, m, m), dtype=np.float64)
        b = np.zeros((n_img, m), dtype=np.float64)
        for a in range(m):
            b[:, a] = np.bincount(img, weights=fw[:, a] * target, minlength=n_img)
            for d in range(a, m):
                e = np.bincount(
                    img, weights=fw[:, a] * feats[:, d], minlength=n_img)
                A[:, a, d] = e
                A[:, d, a] = e
        # Ridge: tiny on the gain (conditioning), gentle vignette-toward-0 prior;
        # the eps keeps under-observed images SPD (they resolve to identity).
        A[:, 0, 0] += lam_gain
        for k in range(v):
            A[:, 1 + k, 1 + k] += lam_vig
        A[:, diag_idx, diag_idx] += 1e-9
        x = np.linalg.solve(A, b[:, :, None])[:, :, 0]
        # Gauge: zero-mean log-gain (the global scale is unobservable; centring
        # it keeps the mean brightness unchanged).
        x[:, 0] -= float(x[:, 0].mean())

        if it + 1 < iters:
            model = np.einsum("om,om->o", feats, x[img])
            resid = (y - model) - ref[trk]
...
def estimate_image_corrections(
    ...
    With the ``mean(alpha)=0`` gauge the gains are centred on 1 (geometric mean),
    ...
    feats = _features(obs.rho2, vignette_order)

    # Per-channel solve.  Saturated/clipped measurements carry no reliable log
    # value → zero base weight (kept in the arrays so indices stay aligned).
    ...
    for c in range(_NUM_CHANNELS):
        ...
        x = _solve_channel(
            feats, y, base_w, obs.img, obs.trk,
            n_img, obs.n_tracks, vignette_order, config)
        gains[:, c] = np.exp(x[:, 0])
        ...
        mask = base_w > 0
        model = np.einsum("om,om->o", feats, x[obs.img])
        rms_before.append(_track_consistency(obs, y, mask))
        rms_after.append(_track_consistency(obs, y - model, mask))
...
def _track_consistency(
    ...
    the raw log-intensity gives the BEFORE figure; passing the corrected
    log-intensity ``logI - (alpha_i + vignette_i)`` gives the AFTER figure — both
    are the spread of the (corrected) colours about the track's common albedo."""
    vals = values[mask]
    trk = obs.trk[mask]
    if vals.size == 0:
        return 0.0
