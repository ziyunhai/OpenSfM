...
// =====================================================================
// svo_carve_vote: Compare rendered depth vs clean depth to vote.
//
// For each pixel:
//   - If clean_depth > rendered_depth * (1 + margin): view sees further
//     -> the rendered surface shouldn't exist -> carve vote.
//   - If |clean_depth - rendered_depth| <= margin * rendered_depth: support vote.
//
// Global work size: (cols_padded, rows_padded)
// =====================================================================
__kernel void svo_carve_vote(
    __global const float* rendered_depth,
    __global const uint*  hit_slot,
    __global const float* clean_depth,
    __global int*         carve_count,   // per-slot carve counter
    __global int*         support_count, // per-slot support counter
    int                   rows,
    int                   cols,
    float                 carve_margin,
    uint                  capacity_mask
    ...
    float rd = rendered_depth[idx];
    float cd = clean_depth[idx];
    uint slot = hit_slot[idx];

    // Need both rendered and clean depth, and a valid hit slot.
    if (rd <= 0.0f || cd <= 0.0f || slot == 0xFFFFFFFFu) return;
    if (slot > capacity_mask) return;

    if (cd > rd * (1.0f + carve_margin)) {
        // View sees further -> carve (rendered surface is false).
        atomic_add(&carve_count[slot], 1);
    } else if (fabs(cd - rd) <= carve_margin * rd) {
        // View agrees -> support.
        atomic_add(&support_count[slot], 1);
    }
    // Otherwise (cd < rd * (1-margin)): foreground occlusion, no info.
}

// =====================================================================
// svo_prune: Remove voxels with too many carve votes and too few support.
//
// Soft pruning: subtract carve-weighted penalty from sum_weight.
// If resulting weight <= 0, mark slot as empty.
//
// Global work size: (capacity)
// =====================================================================
__kernel void svo_prune(
    __global VoxelSlot* table,
    __global const int* carve_count,
    __global const int* support_count,
    uint                capacity,
    int                 carve_threshold,
    int                 support_min,
    int                 weight_penalty   // weight reduction per excess carve vote (WEIGHT_SCALE units)
) {
    uint i = get_global_id(0);
    if (i >= capacity) return;
    ...
    int cv = carve_count[i];
    int sv = support_count[i];

    // Only prune if carve votes exceed threshold AND support is insufficient.
    if (cv > carve_threshold && sv < support_min) {
        int excess = cv - carve_threshold;
        int penalty = excess * weight_penalty;
        int new_weight = table[i].sum_weight - penalty;
        if (new_weight <= 0) {
            // Kill the voxel.
            table[i].key_ab = EMPTY_KEY;
            table[i].key_c = KEY_C_UNINIT;
            ...
            table[i].sum_weight = new_weight;
        }
    }
}

// =====================================================================
// svo_clear_votes: Zero out carve/support counters before a new pass.
// Global work size: (capacity)
// =====================================================================
__kernel void svo_clear_votes(
    __global int* carve_count,
    __global int* support_count,
    uint          capacity
) {
    uint i = get_global_id(0);
    if (i >= capacity) return;
