...
PYBIND11_MODULE(pygeometry, m) {
  ...
  m.def("point_refinement", geometry::PointRefinement,
        py::call_guard<py::gil_scoped_release>());
  m.def("triangulate_bearings_robust", geometry::TriangulateBearingsRobust,
        py::call_guard<py::gil_scoped_release>());
  ...
  // ── OPK / angle-axis conversion utilities ──
  m.def(
      "rotation_from_opk",
      [](double omega, double phi, double kappa) -> Mat3d {
        return geometry::RotationFromOpk(omega, phi, kappa);
      },
      py::arg("omega"), py::arg("phi"), py::arg("kappa"),
      "Build the world-to-camera rotation matrix from OPK angles (radians).");

  m.def(
      "opk_from_rotation",
      [](const Mat3d& rotation_matrix) -> Vec3d {
        return geometry::OpkFromRotation(rotation_matrix);
      },
      py::arg("rotation_matrix"),
      "Extract OPK angles (omega, phi, kappa) in radians from a "
      "world-to-camera rotation matrix.");

  m.def(
      "rotation_from_angle_axis",
      [](const Vec3d& angle_axis) -> Mat3d {
        return geometry::VectorToRotationMatrix(angle_axis);
      },
      py::arg("angle_axis"),
      "Convert an angle-axis vector to a 3x3 rotation matrix.");

  m.def(
      "angle_axis_from_rotation",
      [](const Mat3d& rotation_matrix) -> Vec3d {
        return geometry::RotationMatrixToVector(rotation_matrix);
      },
      py::arg("rotation_matrix"),
      "Convert a 3x3 rotation matrix to an angle-axis vector.");
}
